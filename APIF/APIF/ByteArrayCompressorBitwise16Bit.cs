...
namespace APIF
{
    class ByteArrayCompressorBitwise16Bit
    {
        ...
        public static BitStreamFIFO Compress(AccessibleBitmapBitwise source, int bitLayer)
        {
            ...
            bool[] tmpBools = new bool[12];      //The collection of bits that will be converted to an interger
        ...
        public static AccessibleBitmapBitwise Decompress(BitStreamFIFO inBits, AccessibleBitmapBitwise inBitmap, out BitStreamFIFO restBits, int bitLayer)
        {
            ...
            bool[] tmpBools = new bool[12];  //The array of bits from which bits will be read to be applied to pixels
            int boolIndex = 12;              //The index in the array of bits, from where the next bit should be taken
            ...
            for (int y = 0; y < inBitmap.height; y++)
            {
                ...
                for (int x = 0; x < inBitmap.width; x++)
                {
                    ...
                    if (boolIndex == 12)
                    {
                        ...
                        tmpBools = new BitStreamFIFO().IntToBoolArray(ints[intIndex], 12);
